...
def extrair_dados():
    ...
    ordem_analitos = [
        ["pH","Benzeno", "Etilbenzeno", "Tolueno", "Xilenos"],
        ...
        "Estireno", "Etanol", "Óleos e Graxas"],
        ["TPH C5 - C8 (alifática)", "TPH Fracionado Fração Alifática (C9-C18)", "TPH Fracionado Fração Alifática (C19-C32)",
        "TPH C6 - C8 (aromática)", "TPH Fracionado Fração Aromática (C9-C10)", "TPH Fracionado Fração Aromática (C10-C32)", "TPH Fracionado Fração Aromática (C9-C16)", "TPH Fracionado Fração Aromática (C17-C32)",
        "TPH MCNR", "TPH HRP", "Ftano", "Pristano", "TPH Total (C8-C40)"]
    ]

    ordem_analitos_completa = ordem_analitos[0] + ordem_analitos[1] + ordem_analitos[2]

    for filename in os.listdir(diretorio_entrada):
        if filename.endswith('.xlsx'):
            filepath = os.path.join(diretorio_entrada, filename)
            df = pd.read_excel(filepath, engine='openpyxl', header=None, usecols=None)
            ...
            coluna_A = df.iloc[[14, *range(21, 57), 113], 0]  # Coluna A
            coluna_C = pd.concat([df.iloc[[14, 113], 2], df.iloc[21:57, 3]])  # Coluna C
            coluna_E = pd.concat([df.iloc[[14, 113], 6], df.iloc[21:57, 7]])  # Coluna E
            coluna_J = pd.concat([df.iloc[[14, 113], 10], df.iloc[21:57, 10]])  # Coluna J
    ...
    dados_extraidos.rename(columns={'J': 'VMP - CONAMA nº 420/2009 - Água Subterrânea'}, inplace=True)
