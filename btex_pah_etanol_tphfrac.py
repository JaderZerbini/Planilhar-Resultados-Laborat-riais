...
def extrair_dados():
    ...
    ordem_analitos = [
        ...
        "Criseno", "Benzo(b)fluoranteno", "Benzo(k)fluoranteno", "Benzo(a)pireno", "Indeno(1,2,3-cd)pireno", "Dibenzo(a,h)antraceno",
        "Benzo(g,h,i)perileno", "Estireno", "Ftano", "Pristano", "TPH C5 - C8 (alifática)", "TPH Fracionado Fração Alifática (C9-C18)", "TPH Fracionado Fração Alifática (C19-C32)",
        "TPH C6 - C8 (aromática)", "TPH Fracionado Fração Aromática (C9-C16)", "TPH Fracionado Fração Aromática (C17-C32)", "TPH Total (C8-C40)", "TPH MCNR",
        "TPH HRP", "TPH Fracionado Fração Aromática (C9-C10)", "TPH Fracionado Fração Aromática (C10-C32)"]
    ]
    ...
    for filename in os.listdir(diretorio_entrada):
        if filename.endswith('.xlsx'):
            ...
            coluna_A = df.iloc[[*range(19, 53), *range(107, 108)], 0]  # Coluna A
            coluna_C = df.iloc[[*range(19, 53), *range(107, 108)], 2]  # Coluna C
            coluna_E = df.iloc[[*range(19, 53), *range(107, 108)], 4]  # Coluna E
            coluna_J = df.iloc[[*range(19, 53), *range(107, 108)], 9]  # Coluna J
